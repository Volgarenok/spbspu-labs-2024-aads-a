...
#include <queue> //reinclude after merge S2
...
namespace rebdev
{
  ...
  class AVLTree
  {
    ...
    public:
      ...
      F traverse_lnr(F & f) const
      {
        ...
        while (nodeNow != rightest(headNode_))
        {
          ...
          {
            nodeNow = leftest(nodeNow->right);
          }
          else
          {
            if ((nodeNow->parent->right) == nodeNow)
            {
              while ((nodeNow->parent->right) == nodeNow)
              {
      ...
      F traverse_rnl(F & f) const
      {
        ...
        while (nodeNow != leftest(headNode_->right))
        {
          ...
          {
            nodeNow = rightest(nodeNow->left);
          }
          else
          {
            if ((nodeNow->parent->left) == nodeNow)
            {
              while ((nodeNow->parent->left) == nodeNow)
              {
                nodeNow = (nodeNow->parent);
              }
            }
      ...
      F traverse_breadth(F & f) const
      {
        ...
        {
          nodeNow = nodeQueue.front();
          nodeQueue.pop();
          f(nodeNow->data);
          if ((nodeNow->right) != nullptr)
          {
            nodeQueue.push(nodeNow->right);
          }
          if ((nodeNow->left) != nullptr)
          {
            nodeQueue.push(nodeNow->left);
          }
        }
        return f;
      }
    ...
    private:
      ...
      node * leftest(node * someNode) const noexcept
      {
        node * nodeNow = someNode;
        while ((nodeNow->left) != nullptr)
        {
          nodeNow = (nodeNow->left);
        }
        return nodeNow;
      }
      node * rightest(node * someNode) const noexcept
      {
        node * nodeNow = someNode;
        while ((nodeNow->right) != nullptr)
        {
          nodeNow = (nodeNow->right);
        }
        return nodeNow;
